...
%[fitness, check_matrix, return_scalar, return_vector, total_transit_statematrix] = pso_intermediary(input_vector, lunar_SOI_radius, lunar_orbit_statematrix, latlong_target, target_radius);
...
while iter <= max_iters
    ...
    fprintf("\n completed iter %i, best fitness: %3.4f, best dv: %3.4f km/s.\n",iter,bestfitnesss(iter),global_best_scalar(1)/1e3)
    ...
    if update_plot
        ...
        xlim([global_best_statematrix(1,2)-1e7,global_best_statematrix(1,2)+1e7])
        ylim([global_best_statematrix(1,3)-1e7,global_best_statematrix(1,3)+1e7])
        zlim([global_best_statematrix(1,4)-1e7,global_best_statematrix(1,4)+1e7])
